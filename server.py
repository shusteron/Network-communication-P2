...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        server_socket.bind((host,port)) #assigns an IP address and a port number to a socket instance.
        server_socket.listen(1) #maximum number of connections that can be queued for this socket.
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                client_socket, address = server_socket.accept() #accepts an incoming connection request from a TCP client.
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            data = client_socket.recv(1024) #the maximum length of the message to read at once. 
            if not data:
                break
            try:
                ...
                # * Fill in start (4)
                client_socket.send(bytes(response)) #sending back the data to the another socket.
                # * Fill in end (4)
